...
const Points = () => {
  ...
  const [posInicial, setPosInicial] = useState<[number, number]>([0, 0]);
  ...
  useEffect(() => {
    async function loadPosition() {
      ...
      if (status !== "granted") {
        Alert.alert("Ooooops.....", "Por favor permita localização");
        return;
      }
  ...
  function handleNavigateToDetail(id: number) {
    navigation.navigate("Detail", { idPoint: id });
  }
        ...
        <View style={styles.mapContainer}>
          {posInicial[0] !== 0 && (
            <MapView
              style={styles.map}
              initialRegion={{
                latitude: posInicial[0],
                longitude: posInicial[1],
                latitudeDelta: 0.014,
            ...
            >
              {points.map((item) => (
                <Marker
                  ...
                  onPress={() => handleNavigateToDetail(item.idPoint)}
                  coordinate={{
        ...
        >
          {items.map((item) => (
            <TouchableOpacity
              ...
              style={[
                styles.item,
                selectItem.includes(item.idItem) ? styles.selectedItem : {},
              ]}
              onPress={() => handleSelectItem(item.idItem)}
              activeOpacity={0.6}
            >
